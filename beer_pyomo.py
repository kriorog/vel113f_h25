...
model.A = pyo.Set(initialize=['pilsner', 'vodka', 'brandy', 'malt'])
...
model.x = pyo.Var(model.A, domain=pyo.Reals, bounds=bounds_rule)
...
model.obj_maxAlc = pyo.Objective(
    expr=pyo.sum_product(model.p, model.x),
    sense=pyo.maximize
)

# define the constraint on strong alcohol
model.con_maxStrongAlc = pyo.Constraint(    expr=sum(model.x[i] for i in model.strongAlc) <= model.maxStrongAlc)

# define total volume constraint
model.con_totalVol = pyo.Constraint(expr=pyo.summation(model.x, index=model.A) == model.totalVol)
...
model.con_mix = pyo.Constraint(expr=pyo.sum_product(model.p, model.x) == 4)
