...
$\gamma_{k, j}$  is a real variable between 0 and 1 describing the power output \% of unit $k$ during time period $j$.
...
One does not need both $p$ and $\gamma$ to define the problem - but the variable $\gamma$ provides a quick inspection of the power setting of each unit in each time period between its min and max power values.

## Constraints

### Relationship between $\gamma$ and unit on/off state
The load output ratio $\gamma$ can be any value between $0$ and $1$ if a unit is turned on, but must be zero if it is off.

$$
\gamma_{k, j} \leq x_{k, j} \hspace{0.5 cm} \forall (k, j) \in \mathcal{U} \times \mathcal{T}
$$
...
$$ 
p_{k, j} = x_{k, j} \, \underbar{p}_k + \gamma_{k, j} \left( \overbar{p}_k - \underbar{p}_k \ \right)
$$

If a unit is turned on, then $x_{k, j}=1$, and the constraint becomes
$$
p_{k, j} = \underbar{p}_k + \gamma_{k, j} \left( \overbar{p}_k - \underbar{p}_k \ \right)
$$
which yields a power output between the defined lower and upper bound for each unit $k$.

If the unit is off, then $x_{k, j} = 0$, and the constraint becomes $p_{k, j} = 0$. If we had not included $x_{k, j}$ in the first element on the right hand side of the constraint, we would not force a turned off unit to produce zero power.
