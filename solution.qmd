...
Using a locally installed version of GLPK does return the duals. Note also that duals can only be returned for a linear problem, not for a problem with integer variables. 
...
- For the lower bound, only the dual associated with index $(k,j)=(1,2)=(1,7)$ is non-zero, with a value of -0.5 kkr/MW.
    ...
    This dual applies to the lower bound on produced power of unit 1, so reducing the lower bound would reduce the total cost. Since unit 3 only has a 5 MW power slack, then reducing the lower limit of unit 1 by 5 MW is the best we can do. 
...
- Index $(k,j)=(3,6)$ is negative of value -0.5 kkr/MW. From the solution we see that unit 3 is off in this time period, since it is running in the next 3 consecutive periods. Unit 1 is supplying the entire power demand, having been kept running from period 5. Allowing unit 3 to run for one more consecutive period (max 4) would reduce cost. Total power demand is 50 MW (unit 3 max is 55 MW), so unit 3 only produces 50 MW. Total cost reduction is $-0.5 \cdot 50 = -25 \text{ kkr}$.

- Next indices with negative duals are $(k,j)=(3,3)=(3,8)$ and $(k,j)=(3,4)=(3,9)$ which both have a dual with value of -0.5 kkr/MW. Unit 3 is on in both periods, producing max power of 55 MW. Unit 2 is also on and provides the surplus power to meet demand, i.e. producing 25 MW in period 8 and 15 MW in period 9. This dual indicates that raising the upper power limit on unit 3 allows us to take advantage of the lower running cost of unit 3. 

    To fully transfer all load to unit 3, its capability must be increased by at least 25 MW. Doing so would reduce total cost by $-0.5 \cdot 25 = 12.5 \text{ kkr}$ in each period, a total of -25 kkr for both.
...
- Final index with negative dual is $(k,j)=(3,5)=(3,10)$ with value -0.4 kkr/MW. Unit 3 is off in this period since it was running for the previous 3 periods. Power demand is supplied by units 1 and 2, with unit 1 started to do so. Unit 2 produces its minimum and is kept running from the previous period. This dual tells us that allowing unit 3 to run for one addition period (max 4 consecutive) would reduce cost. Power demand is 60 MW and unit 3 capacity is 55 MW, so we either keep unit 1 or 2 running in parallel. Since unit 1 has lower min power, we can keep unit 3 at a higher power output of 50 MW and unit 1 at 10 MW.

    Therefore, for a 50 MW increase in unit 3 output during period 5 (10), the associated cost reduction is $-0.4 \cdot 50 = -20 \text{ kkr}$. We also notice that in doing this, each start of unit 3 changes from cold to warm, with associated cost reducton of $-0.5 \cdot 16=-8 \text{ kkr}$. Total cost reduction is thus estimated as -28 kkr.
...
- The duals do not account for possible changes in the number of starts required for each unit, and the temperature of starts (warm/cold).

- The duals do not take into account effects on cyclicity.

- Even though the duals did not exactly estimate the cost reduction, they did identify the improvement strategy and provided a lower bound on the improvement.

Inspecting the solution to the modified problem (max 4 continuous periods), we see that units 1 and 3 both run for the maximum 4 periods, while unit 1 is the slack unit. For running unit 3 for max 4 consecutive periods, we estimated a cost reduction of -28 kkr. This change has also changed a cold start of unit 1 to a warm start, with an additional cost reduction of $-0.5 \cdot 10 = -5 \text{ kkr}$. Therefore, the total cost reduction would be -33 kkr, the same we obtained in the modified model. The total reduction in production cost associated with allowing this change (33 kkr) must therefore be greater than the cost of running units 1 and 3 for longer.

Allowing max 5 or more consecutive periods is not possible due to the cyclic constraint and a 5 period schedule. 

Note that we can still apply the changes to the upper and lower power output limits of unit 1 and 3 respectively to obtain a further cost reduction.